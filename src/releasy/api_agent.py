...
def _tool_defs(names: set[str], prefixes: list[str] | None) -> list[dict]:
    ...
    # Basic variants on purpose. The ``_20260209`` versions filter results
    # through code execution, which makes the API classify the request as
    # "code execution with tools" and reject the next tool-result request
    # with "container_id is required when there are pending tool uses
    # generated by code execution" — while never surfacing a container id
    # on the response for us to send back. The basic versions have no
    # dynamic filtering (so searches cost more tokens) and no container.
    if "WebSearch" in names:
        defs.append({"type": "web_search_20250305", "name": "web_search"})
    if "WebFetch" in names:
        defs.append({"type": "web_fetch_20250910", "name": "web_fetch"})
    return defs
...
_OPTIONAL_PARAMS = ("thinking", "output_config", "cache_control", "container")
...
def run_agent(
    ...
    timed_out = False
    subtype = "success"
    # Server-side container the turn ran code execution in. The
    # ``_20260209`` web tools filter results through code execution, so a
    # turn can acquire one without us ever declaring that tool — and once
    # it has, every follow-up request carrying pending tool uses must name
    # it ("container_id is required when there are pending tool uses
    ...
    try:
        while True:
            ...
            turns += 1
            # Sticky: a later turn that doesn't touch code execution reports
            # no container, but the one it opened is still alive and still
            # required by the next tool-result request. Logged the first
            # time we see one — whether a container is ever surfaced at all
            # is the fact that distinguishes a fixable id-threading bug from
            # a tool set we simply shouldn't be declaring.
            seen = getattr(getattr(response, "container", None), "id", None)
            if seen and seen != container_id:
                log.info("api-agent: code-execution container %s", seen)
            container_id = seen or container_id
