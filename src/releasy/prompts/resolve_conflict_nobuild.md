# Claude skill: resolve a RelEasy port cherry-pick conflict (resolve-only)
...
> **NOTE:** One step of a larger pipeline. **You resolve only — you do
> NOT build.** Done when a NEW commit on top of `{pre_resolve_sha}` holds
> a clean, marker-free resolution and `git status` is clean. RelEasy then
> runs the build and the PR's tests itself; if the build breaks, it will
> call you back with the build log to fix it. So resolve faithfully and
> stop — **do not run any build (`{build_command}` / `ninja` / `cmake` /
> `make`), do not push, do not run `gh pr ...` mutations.**
...
> **Do NOT build.** RelEasy compiles the result itself the moment you
> finish. If it fails to build, RelEasy calls you back in a separate
> step with the build-log tail to fix — that is not your job now.
...
- **Do not build.** No `{build_command}` / `ninja` / `cmake` / `make`.
  RelEasy builds after you finish.
...
- On success, final line must be `RESOLVED`.
